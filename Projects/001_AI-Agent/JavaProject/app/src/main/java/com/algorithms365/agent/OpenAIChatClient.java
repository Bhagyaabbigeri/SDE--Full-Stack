...
public class OpenAIChatClient {
    ...
    public OpenAIChatClient(ChatModel defaultModel) {
        // OpenAIOkHttpClient.fromEnv() in this SDK version takes no arguments.
        // Keep default timeouts from environment/client configuration.
    ...
    public String complete(String prompt, Long maxTokens) {
        try {
            ...
            java.util.Optional<String> contentOpt = completion.choices().get(0).message().content();
            String text = contentOpt.orElseThrow(() -> new RuntimeException("OpenAI response content was null."));
            return text.trim();
